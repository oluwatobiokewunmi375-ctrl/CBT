...
Locator: locator('tr').filter({ has: locator('text=STU001') }).locator('button').filter({ hasText: /Reset Password/i })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('tr').filter({ has: locator('text=STU001') }).locator('button').filter({ hasText: /Reset Password/i })

```

```yaml
- heading "👥 Students Management" [level=1]
- paragraph: Manage student accounts and enrollment
- button "Add Student"
- paragraph: Total Students
- paragraph: "0"
- paragraph: Active This Term
- paragraph: "0"
- paragraph: Average Performance
- paragraph: 78.5%
- paragraph: No students yet. Add one to get started.
- alert
```
